...
function Navbar({ buttonVariant, isUnderlined, buttonTextSize, textColor }: NavProps) {
    return (
        <div className={`w-full flex items-center justify-between px-32 absolute top-0 left-0 z-20 ${isUnderlined ? 'border-b' : ""}`}>
            ...
            <div className=" h-[100px] w-[150px]">
                <Link href={"/"}><Image src="/netflix-logo.svg" alt='logo' width={150} height={100} className='object-cover max-w-full max-h-full' quality={100} /></Link>
            </div>
            {/* SIGN IN BUTTON */}
            <div>
                <Button className={`font-bold text-${buttonTextSize} text-${textColor ? textColor : 'white'}`} size={"default"} variant={buttonVariant === "ghost" ? "ghost" : "default"}>Sign In</Button>
            </div>
        </div>
    )
}
